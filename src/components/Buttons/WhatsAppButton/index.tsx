...
import type {ButtonStyle, ComponentSize, WhatsAppButtonProps} from '@/types/whatsapp';
...
const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({
    ...
    const styleClasses: Record<ButtonStyle, string> = {
        filled: 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white shadow-lg',
    ...
    return (
        <motion.div
            className="fixed z-80 w-full h-full"
            variants={positionVariants}
            initial="defaultPos"
            animate={scrolled ? 'scrolledPos' : mobile ? 'mobile' : 'defaultPos'}
        >
            <motion.button
            onClick={handleClick}
            disabled={disabled}
      ...
      `}
            ...
            aria-label={ariaLabel || getButtonText() || 'Contato via WhatsApp'}
            aria-disabled={disabled}
        >
                {/* Background pulse effect for pulse animation */}
            {animation === 'pulse' && (
                <motion.div
