...
interface WhatsAppIconProps {
    variant?: 'floating' | 'pulse' | 'breathing' | 'fixed';
    size?: 'sm' | 'md' | 'lg' | 'xl';
    phoneNumber?: string;
    message?: string;
    className?: string;
    ...
    return (
        ...
        `}
            ...
            >
                ...
                <svg
                    className="w-3/5 h-3/5 text-white relative z-10 group-hover:scale-110 transition-transform duration-300"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                >
                    <path
                ...
                <div
                    className="absolute inset-0 rounded-full bg-white opacity-0 group-active:opacity-30 group-active:animate-ping"></div>
            </button>

            {/* Tooltip for fixed variant */}
            {variant === 'fixed' && (
                <div
                    ...
                    Chat on WhatsApp
                    <div
                        className="absolute left-full top-1/2 transform -translate-y-1/2 border-4 border-transparent border-l-gray-800"></div>
                </div>
            )}
        </div>
    );
